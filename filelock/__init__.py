...
import sys
warnings: Optional[ModuleType] = None
try:
    import warnings # type: ignore[no-redef]
except ImportError:
    pass
...
def logger() -> logging.Logger:
    ...
    global _logger
    _logger = _logger or logging.getLogger(__name__) #type: ignore[unreachable]
    return _logger
...
class _Acquire_ReturnProxy(object):
    ...
    def __init__(self, lock: "BaseFileLock"):
        self.lock = lock
        return None
...
class BaseFileLock(object):
    ...
    def acquire(
        self, timeout: Optional[float] = None, poll_intervall: float = 0.05
...
if TYPE_CHECKING:
    ...
    # Can't rely on detecting whether msvcrt exists because it's present in typeshed, which
    # means it's always present for mypy
